...
// Favorite Users
        ...
        void getFavoriteUsersNames(StringSet& p_users) const // TODO оптимизировать упаковку в уникальные ники отложенно в момент измения базовой мапы users.
        {
            ...
            {
                if (!i->second.getNick().empty()) // Ники могут дублировать и могут быть пустыми. https://www.box.com/shared/p4p0scdeh92wpxf9bohi
                {
...
// User Commands
        ...
        void addIgnoredUser(const CID& cid)
        {
            Lock l(cs);
            ignoredUsers.insert(cid);
        }
        void removeIgnoredUser(const CID& cid)
        {
            Lock l(cs);
            ignoredUsers.erase(cid);
        }
        bool isIgnoredUser(const CID& cid) const
        {
            Lock l(cs);
            return ignoredUsers.find(cid) != ignoredUsers.end();
        }
        unordered_set<CID> getIgnoredUsers() const
        {
            Lock l(cs);
            return ignoredUsers;
        }
