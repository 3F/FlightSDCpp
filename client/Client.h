...
class Client : public ClientBase, public Speaker<ClientListener>, public BufferedSocketListener, protected TimerManagerListener
{
	public:
		...
		{
			return sock && state != STATE_DISCONNECTED;
		}
		bool isReady() const
		{
			return sock && state != STATE_CONNECTING && state != STATE_DISCONNECTED;
		}
		bool isSecure() const;
		...
		{
			return getIp() + ':' + Util::toString(port);
		}
		...
		{
			return getHubIdentity().getNick().empty() ? getHubUrl() : getHubIdentity().getNick();
		}
	...
	protected:
		...
		static boost::atomic<long> counts[COUNT_UNCOUNTED];
