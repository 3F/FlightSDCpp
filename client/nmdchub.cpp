...
void NmdcHub::updateFromTag(Identity& id, const string& tag)
{
	...
	for (auto i = tok.getTokens().cbegin(); i != tok.getTokens().cend(); ++i)
	{
		...
		else if (i->compare(0, 2, "S:") == 0)
		{
			...
			// TODO Fly if (id.getUser())
			// TODO Fly id.getUser()->setCountSlots(slots); //[+]PPA
		}
		else if ((j = i->find("V:")) != string::npos)
		{
			id.set("VE", i->c_str() + j + 2);
		}
		...
		{
			const int l_limit = Util::toInt(i->c_str() + j + 2);
			id.set("US", Util::toString(l_limit * 1024));
		}
	}
...
void NmdcHub::search(int aSizeType, int64_t aSize, int aFileType, const string& aString, const string&, const StringList&)
{
	...
	string tmp = ((aFileType == SearchManager::TYPE_TTH) ? g_tth + aString : fromUtf8(escape(aString)));
	string::size_type i;
