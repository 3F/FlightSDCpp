...
Client* ClientManager::getClient(const string& aHubURL)
{
	...
	{
		Lock l(cs);
		clients.insert(make_pair(c->getHubUrl(), c));
	}
...
StringList ClientManager::getNicks(const CID& p_cid, const string& hintUrl, bool priv) const
{
	...
	if (!priv)
	{
		OnlinePairC op = onlineUsers.equal_range(p_cid);
		for (OnlineIterC i = op.first; i != op.second; ++i)
		{
			if (i->second) //{+]PPA
	...
	{
		OnlineUser* u = findOnlineUserHint(p_cid, hintUrl);
		if (u)
	...
	{
		// offline
		NickMap::const_iterator i = nicks.find(p_cid);
		if (i != nicks.end())
		{
			ret.insert(i->second);
		}
                       ...
                       int aFileType, const string& aString, bool isPassive) noexcept
{
	...
	else if (!isPassive && (aFileType == SearchManager::TYPE_TTH) && (aString.compare(0, 4, g_tth) == 0))
	{
...
void ClientManager::on(AdcSearch, const Client* c, const AdcCommand& adc, const CID& from) noexcept
{
	bool isUdpActive = false;
	{
		...
		OnlinePairC op = onlineUsers.equal_range((from));
		for (OnlineIterC i = op.first; i != op.second; ++i)
		{
...
void ClientManager::on(TimerManagerListener::Minute, uint64_t /*aTick*/) noexcept
{
	...
	while (i != users.end())
	{
		if (i->second->unique())
		{
			NickMap::iterator n = nicks.find(i->second->getCID());
...
void ClientManager::updateNick(const UserPtr& user, const string& nick) noexcept
{
	...
	{
		Lock l(cs);
		NickMap::iterator i = nicks.find(user->getCID());
		if (i == nicks.end())
		{
			nicks[user->getCID()] = nick;
		}
		else
		{
			i->second = nick;
