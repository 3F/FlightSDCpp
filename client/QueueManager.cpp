    ...
    do
    {
        ...
        if (i != userQueue[p].end())
        {
            ...
            for (auto j = i->second.begin(); j != i->second.end(); ++j)
            {
                ...
                if (!qi->getDownloads().empty() && qi->getDownloads().front()->getType() == Transfer::TYPE_TREE)
                {
                    continue;
                }
                if (!qi->isSet(QueueItem::FLAG_USER_LIST))
...
{
    auto i = m_running.find(aUser);
    return i == m_running.end() ? 0 : i->second;
}

void QueueManager::UserQueue::remove(QueueItem* qi, bool removeRunning)
{
    for (auto i = qi->getSources().cbegin(); i != qi->getSources().cend(); ++i)
    {
...
{
    Lock l(cs);
    if (QueueItem* qi = fileQueue.find(name))
{
tth = qi->getTTH();
    return true;
}
...
void QueueManager::on(TimerManagerListener::Minute, uint64_t aTick) noexcept
{
    ...
    
    {
        ...
        if (BOOLSETTING(AUTO_SEARCH) && (aTick >= nextSearch) && (fileQueue.getSize() > 0))
        {
            ...
            while ((qi = fileQueue.findAutoSearch(recent)) == nullptr && !recent.empty()) // Местами не переставялть. findAutoSearch меняет recent
            {
                recent.pop_front();
            }
            if (qi != nullptr)
...
//[-]PPA
    ...
    */
    ...
    
    {
        ...
        if (q == nullptr && !(aFlags & QueueItem::FLAG_USER_LIST))
        {
            QueueItemList ql;
...
void QueueManager::processList(const string& name, const HintedUser& user, int flags)
{
    ...
    if (flags & QueueItem::FLAG_DIRECTORY_DOWNLOAD)
    {
        ...
        {
            DirectoryItem* di = *i;
            dirList.download(di->getName(), di->getTarget(), false, di->getPriority()); //todo di->getName() IS NULL
            delete di;
        }
    }
...
void QueueManager::saveQueue(bool force) noexcept
{
    ...
    {
        LogManager::getInstance()->getInstance()->message("[QueueManager::saveQueue] error: " + e.getError());
    }
