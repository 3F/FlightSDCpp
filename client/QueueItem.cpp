...
static const std::string TEMP_EXTENSION = ".dctmp";

static std::string getTempName(const std::string& aFileName, const TTHValue& aRoot)
{
	std::string tmp(aFileName);
	tmp += "." + aRoot.toBase32();
	tmp += TEMP_EXTENSION;
	return tmp;
}
...
QueueItem::QueueItem(const std::string& aTarget, int64_t aSize, Priority aPriority, Flags::MaskType aFlag,
                     time_t aAdded, const TTHValue& p_tth) :
	target(aTarget), maxSegments(1), fileBegin(0),
	size(aSize), priority(aPriority), added(aAdded),
	m_tthRoot(p_tth), autoPriority(false), nextPublishingTime(0),
//	m_dirty(true),
	m_block_size(0)
//	m_downloadedBytes(0),
//	m_averageSpeed(0)
