...
void File_Wm::Data_Packet()
{
    ...
    for (NumberPayloads_Pos=0; NumberPayloads_Pos<NumberPayloads; NumberPayloads_Pos++)
    {
...
#ifdef _DEBUG
		const int32u l_Stream_Number =  Stream_Number; // [+]FlylinkDC++ Team
#endif
		stream& l_StreamStream_Number = Stream[Stream_Number]; // [+]FlylinkDC++ Team
        if (l_StreamStream_Number.Parser && l_StreamStream_Number.SearchingPayload)
        {
            ...
            //Codec specific
            #if defined(MEDIAINFO_VC1_YES)
            if (Retrieve(l_StreamStream_Number.StreamKind, l_StreamStream_Number.StreamPos, Fill_Parameter(l_StreamStream_Number.StreamKind, Generic_Format))==__T("VC-1"))
                ((File_Vc1*)l_StreamStream_Number.Parser)->FrameIsAlwaysComplete=FrameIsAlwaysComplete;
            #endif

            Open_Buffer_Continue(l_StreamStream_Number.Parser, (size_t)PayloadLength);
			assert(l_Stream_Number == Stream_Number);// [+]FlylinkDC++ Team
            if (l_StreamStream_Number.Parser->Status[IsFinished]
             || (l_StreamStream_Number.PresentationTime_Count>=300 && MediaInfoLib::Config.ParseSpeed_Get()<1))
            {
                l_StreamStream_Number.Parser->Open_Buffer_Unsynch();
                l_StreamStream_Number.SearchingPayload=false;
                Streams_Count--;
            }
        ...
        else
        {
			...
			assert(l_Stream_Number == Stream_Number);// [+]FlylinkDC++ Team
            if (l_StreamStream_Number.SearchingPayload
             && (l_StreamStream_Number.StreamKind==Stream_Video && l_StreamStream_Number.PresentationTime_Count>=300))
            {
                l_StreamStream_Number.SearchingPayload=false;
                Streams_Count--;
            }
...
#endif
	stream& l_StreamStream_Number = Stream[Stream_Number]; // [+]FlylinkDC++ Team
    for (size_t Pos=0; Pos<l_StreamStream_Number.Payload_Extension_Systems.size(); Pos++)
    {
        Element_Begin0();
		assert(l_Stream_Number == Stream_Number);// [+]FlylinkDC++ Team
        switch (l_StreamStream_Number.Payload_Extension_Systems[Pos].ID.hi)
        {
            ...
            default :                                               //Not enough info to validate this algorithm
                                                                    ...
                                                                    //}
                                                                    //else
                                                                        Pos=l_StreamStream_Number.Payload_Extension_Systems.size(); //Disabling the rest, all is unknown
        }
        Element_End0();
    }
