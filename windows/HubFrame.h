    ...
    public UCHandler<HubFrame>, public UserInfoBaseHandler<HubFrame>, private SettingsManagerListener
{
    public:
        ...
        TypedListViewCtrl<OnlineUser, IDC_USERS>& getUserList()
        {
            return ctrlUsers;
        }
    ...
    private:
        ...
        {
            MessageTask(const Identity& from_, const string& m) : StringTask(m), from(from_), hub(false), bot(false) { }
        ...
        void speak(Tasks s)
        {
            tasks.add(static_cast<uint8_t>(s), 0);
            PostMessage(WM_SPEAKER);
        }
        void speak(Tasks s, const string& msg, bool inChat = true)
        {
            tasks.add(static_cast<uint8_t>(s), unique_ptr<Task>(new StatusTask(msg, inChat)));
            PostMessage(WM_SPEAKER);
        }
        void speak(Tasks s, const OnlineUserPtr& u)
        {
            tasks.add(static_cast<uint8_t>(s), unique_ptr<Task>(new UserTask(u)));
            updateUsers = true;
        }
        void speak(Tasks s, const Identity& from, const string& line)
        {
            tasks.add(static_cast<uint8_t>(s), unique_ptr<Task>(new MessageTask(from, line)));
            PostMessage(WM_SPEAKER);
        }
        void speak(Tasks s, const OnlineUserPtr& from, const OnlineUserPtr& to, const OnlineUserPtr& replyTo, const string& line)
        {
            tasks.add(static_cast<uint8_t>(s), unique_ptr<Task>(new MessageTask(from->getIdentity(), to, replyTo, line)));
            PostMessage(WM_SPEAKER);
        }
};
