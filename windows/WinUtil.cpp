...
void WinUtil::init(HWND hWnd)
{
	...
	file.AppendMenu(MF_STRING, IDC_FLYLINK_DISCOVER, CTSTRING(MENU_IDC_FLYLINK_DISCOVER));
#endif
...
{
	const StringTokenizer<tstring> st(setting, _T(','));
...
{
	const StringTokenizer<string> t(tokens, _T(','));
	const StringList& l = t.getTokens();
	if (maxItems == -1)
		maxItems = l.size();
		
	int k = 0;
	for (auto i = l.cbegin(); i != l.cend() && k < maxItems; ++i, ++k)
	{
...
#define LINE2 _T("-- http://flylinkdc.blogspot.com/  <StrongDC++ sqlite ") T_VERSIONSTRING _T(" / ") _T(DCVERSIONSTRING) _T(">")
...
void WinUtil::parseMagnetUri(const tstring& aUrl, bool /*aOverride*/)
{
	...
	if (strnicmp(aUrl.c_str(), _T("magnet:?"), 8) == 0)
	{
		...
		const StringTokenizer<tstring> mag(aUrl.substr(8), _T('&'));
...
uint8_t WinUtil::getFlagIndexByName(const char* countryName)
{
	...
	const char** i = find_if(first, last, [&](const char * cn)
	{
		return stricmp(countryName, cn) == 0;
	});
	if (i != last)
...
void WinUtil::loadReBarSettings(HWND bar)
{
	...
	const StringTokenizer<string> st(SETTING(TOOLBAR_SETTINGS), ';');
	const StringList &sl = st.getTokens();
	
	for (auto i = sl.cbegin(); i != sl.cend(); ++i)
	{
		const StringTokenizer<string> stBar(*i, ',');
		const StringList &slBar = stBar.getTokens();
