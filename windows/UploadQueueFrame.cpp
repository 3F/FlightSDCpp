...
LRESULT UploadQueueFrame::onItemChanged(int /*idCtrl*/, LPNMHDR /* pnmh */, BOOL& /*bHandled*/)
{
    ...
    while (userNode)
    {
        ...
        {
            auto users = UploadManager::getInstance()->getUploadQueue();
            auto it = find_if(users.begin(), users.end(), [&](const UserPtr & u)
            {
                return u == ui->u;
            });
            if (it != users.end())
...
const tstring UploadQueueItem::getText(uint8_t col) const
{
    ...
    {
        case COLUMN_FILE:
            return Text::toT(Util::getFileName(file));
        case COLUMN_PATH:
            return Text::toT(Util::getFilePath(file));
        case COLUMN_NICK:
        ...
        case COLUMN_ADDED:
            return Text::toT(Util::formatTime("%Y-%m-%d %H:%M", time));
        case COLUMN_TRANSFERRED:
            return Util::formatBytesW(pos) + _T(" (") + Util::toStringW((double)pos * 100.0 / (double)size) + _T("%)");
        case COLUMN_WAITING:
            return Util::formatSeconds(GET_TIME() - time);
        default:
