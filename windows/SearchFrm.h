	...
	private SettingsManagerListener, private TimerManagerListener
{
	public:
		...
		void setInitial(const tstring& str, LONGLONG size, SearchManager::SizeModes mode, SearchManager::TypeModes type)
		{
			initialString = str;
			initialSize = size;
			initialMode = mode;
		...
		static const TStringSet& getLastSearches()
		{
			return lastSearches;
		}
    ...
    protected:
         ...
         * see detail: 0fc7354 (~7ms - 10000rc) & current: ~5ms
         */
        bool matchWildcards(const tstring& text, const tstring& filter);
        /** support ^ */
        #ifdef _DEBUG
            void matchWildcardsTests();
        #endif
			...
			public:
				...
				const tstring getText(uint8_t col) const
				{
					...
					switch (col)
					{
						...
						case COLUMN_NICK:
							return WinUtil::getNicks(sr->getUser(), sr->getHubURL());
						case COLUMN_TYPE:
							if (sr->getType() == SearchResult::TYPE_FILE)
							{
...
//						else
									...
									else if (CFlylinkDBManager::getInstance()->is_old_tth(sr->getTTH()))
						...
						case COLUMN_CONNECTION:
							return Text::toT(ClientManager::getInstance()->getConnection(getUser()->getCID()));
						case COLUMN_HUB:
				...
				inline SearchInfo* createParent()
				{
					return this;
				}
				inline const TTHValue& getGroupCond() const
				{
					return sr->getTTH();
				}
