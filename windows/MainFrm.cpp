...
MainFrame::MainFrame() : trayMessage(0), maximized(false), lastUpload(-1), lastUpdate(0),
	lastUp(0), lastDown(0), oldshutdown(false), stopperThread(NULL), m_normalicon(NULL), m_pmicon(NULL),
	tbarcreated(false),
	hShutdownIcon(NULL),
	lastMove(NULL),
#ifdef PPA_INCLUDE_VERSION_CHECK
...
{
	ListMatcher* matcher = new ListMatcher(dcpp::File::findFiles(Util::getListPath(), "*.xml*"));
	try
	{
		matcher->start();
	}
...
LRESULT MainFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	try
	{
		dcpp::File::ensureDirectory(SETTING(LOG_DIRECTORY));
	}
	...
	{
		LogManager::getInstance()->getInstance()->message(e.getError());
	}
...
LRESULT MainFrame::onSpeaker(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
{
	...
	else if (wParam == VIEW_FILE_AND_DELETE)
	{
		...
		dcpp::File::deleteFile(Text::fromT(*file));
	}
...
LRESULT MainFrame::onLink(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	switch (wID)
	{
		case IDC_HELP_HOMEPAGE:
			site = _T("http://flylinkdc.blogspot.com/");
			break;
		case IDC_HELP_SOURCE_CODE:
			site = _T("http://code.google.com/p/flylinkdc/");
			break;
		case IDC_HELP_GEOIPFILE:
			site = _T("http://www.maxmind.com/download/geoip/database/GeoIPCountryCSV.zip");
			break;
		case IDC_HELP_TRANSLATIONS:
			site = _T("http://strongdc.sf.net/forum/viewtopic.php?t=5201");
			break;
		case IDC_HELP_FAQ:
			site = _T("http://strongdc.sf.net/forum/viewtopic.php?t=4067");
			break;
		case IDC_HELP_DISCUSS:
			site = _T("http://groups.google.com/group/flylinkdc/");
			break;
		default:
			dcassert(0);
	}
...
int MainFrame::run()
{
	...
	if (WinUtil::browseFile(file, m_hWnd, false, lastTTHdir) == IDOK)
	{
		...
		std::unique_ptr<char[]> buf(new char[512 * 1024]);
		
		try
		{
			dcpp::File f(Text::fromT(file), dcpp::File::READ, dcpp::File::OPEN);
...
LRESULT MainFrame::onOpenFileList(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	if (wID == IDC_OPEN_MY_LIST)
	{
		...
		{
			DirectoryListingFrame::openWindow(Text::toT(ShareManager::getInstance()->getOwnListFile()),
			                                  Text::toT(Util::emptyString), HintedUser(ClientManager::getInstance()->getMe(), Util::emptyString), 0);
		}
		return 0;
	}
