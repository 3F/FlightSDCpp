    ...
    
{
    public:
        ...
        UserListHandler getUserList()
        {
            return UserListHandler(*dl.get());
        }
                ...
                const tstring getText(uint8_t col) const
                {
                    switch (col)
                    {
                        case COLUMN_FILENAME:
                            return type == DIRECTORY ? Text::toT(dir->getName()) : Text::toT(file->getName());
                        case COLUMN_TYPE:
                            if (type == FILE)
                            {
                        ...
                        case COLUMN_EXACTSIZE:
                            return type == DIRECTORY ? Util::formatExactSize(dir->getTotalSize()) : Util::formatExactSize(file->getSize());
                        case COLUMN_SIZE:
                            return  type == DIRECTORY ? Util::formatBytesW(dir->getTotalSize()) : Util::formatBytesW(file->getSize());
                        case COLUMN_PATH:
                        {
                            ...
                            if (type == FILE && l_result.empty())
                                ...
                                else if (file->isSet(DirectoryListing::FLAG_OLD_TTH))
                                    l_result = _T("Этот файл был у меня в шаре...");
                            return l_result;
                        }
                        case COLUMN_TTH:
                            return type == FILE ? Text::toT(file->getTTH().toBase32()) : Util::emptyStringT;
                        case COLUMN_HIT:
                        ...
                        case COLUMN_TS:
                        {
                            ...
                            else if (type == DIRECTORY) //[!] fix r349
                ...
                static int compareItems(const ItemInfo* a, const ItemInfo* b, uint8_t col)
                {
                    ...
                    else
                    {
                        switch (col)
                        {
                            ...
                            default:
                                return lstrcmp(a->getText(col).c_str(), b->getText(col).c_str());
                        }
                    }
                }
