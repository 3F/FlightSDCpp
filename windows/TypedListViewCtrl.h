    ...
    public ListViewArrows<TypedListViewCtrl<T, ctrlId> >
{
    public:
        ...
        ~TypedListViewCtrl()
        {
            for_each(columnList.begin(), columnList.end(), DeleteFunction());
        }
        ...
        inline void setText(LVITEM& i, const tstring& text)
        {
            lstrcpyn(i.pszText, text.c_str(), i.cchTextMax);
        }
        inline void setText(LVITEM& i, const TCHAR* text)
        {
        ...
        void setVisible(const std::string& vis)
        {
            const StringTokenizer<std::string> tok(vis, ',');
            const StringList l = tok.getTokens();
            
            auto i = l.cbegin();
            for (auto j = columnList.cbegin(); j != columnList.cend() && i != l.end(); ++i, ++j)
            {
        ...
        inline uint8_t findColumn(int col) const
        {
    ...
    public:
        ...
        void deleteAllItems()
        {
            ...
            for (auto i = parents.begin(); i != parents.end(); ++i)
            {
                ...
                for (auto j = (*i).second.children.begin(); j != (*i).second.children.end(); ++j)
                {
                    deleteItem(*j);
                    delete *j;
                }
